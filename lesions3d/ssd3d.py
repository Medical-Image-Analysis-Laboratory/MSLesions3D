...
class MobileNetBase(nn.Module):
    def __init__(self, in_channels=1, width_mult=1.):
        ...
        for c, n, s in cfg:
            if max(ASPECT_RATIOS.keys()) < 6 and c > 256:
                continue
...
class LSSD3D(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.n_classes = n_classes
        self.base = MobileNetBase(in_channels=input_channels, width_mult=width_mult)
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.current_epoch % self.compute_metric_every_n_epochs == 0:
            avg_mAP = torch.stack([x["log"]["val_mAP"] for x in outputs]).mean()
            self.logger.experiment.add_scalar('mAP/validation', avg_mAP, self.global_step)
    ...
    def training_epoch_end(self, outputs):
        # Log the mAP every n epochs
        if self.current_epoch % (self.compute_metric_every_n_epochs * 2) == 0:
            avg_mAP = torch.stack([x["log"]["train_mAP"] for x in outputs]).mean()
            self.logger.experiment.add_scalar('mAP/training', avg_mAP, self.global_step)
        ...
        for i in range(batch_size):
            ...
            if self.thresholding_mode == "hard":
                ...
                label_for_each_prior[(overlap_for_each_prior >= self.threshold[0]) &
                                     (overlap_for_each_prior < self.threshold[1])] = -1
