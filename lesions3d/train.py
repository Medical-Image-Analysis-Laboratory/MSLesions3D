...
def example():
    ...
    comments = """Testing WandB """

    model = LSSD3D(n_classes=n_classes + 1, input_channels=1, lr=5e-4,  # lr=5e-3,
                   width_mult=0.4, scheduler="none", batch_size=batch_size,
                   comments=comments, compute_metric_every_n_epochs=5, use_wandb=False)
    model.init()

    # dataset = ExampleDataset(n_classes = n_classes, subject = "0420", percentage = -1, cache=True, num_workers=8, objects="multiple", batch_size=batch_size)
    # dataset = ExampleDataset(n_classes = n_classes, percentage = 1., cache=True, num_workers=8, objects="multiple", batch_size=batch_size)
    # dataset.setup(stage="fit")
    ...
    trainer = pl.Trainer(gpus=1, max_epochs=600, fast_dev_run=False, logger=logger,  #
